...
export class ConnectionProfileService {
  readonly _friendRequests$ = new BehaviorSubject<FriendRequest[]>(null);
  ...
  getConnectionUser(id: number): Observable<User> {
    return this.http.get<User>(
      `${environment.baseApiUrl}/user/${id}`
    );
  }
  ...
  getFriendRequests(): Observable<FriendRequest[]> {
    return this.http.get<FriendRequest[]>(
      ...
      tap((friendRequests) => { this._friendRequests$.next(friendRequests) })
    );
  }
